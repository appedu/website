    ...
    <div class='.col-md-12'>
        <div class="btn-group pull-right">
            ...
            echo $this->Form->postLink(__(''), array('action' => 'delete',
                $this->Form->value('StudentsSubject.id')), array('class' => 'icon-trash btn btn-danger'), __('Are you sure you want to delete # %s?', $this->Form->value('StudentsSubject.id')));
            ?>

            <?php echo $this->Html->link(__(''), array('action' => 'index'), array('class' => 'btn icon-undo')); ?>
        </div><!-- /.btn-group pull-right -->
        <h2><?php echo __('Admin Edit Student Subjects'); ?></h2>
    </div>

    <div class="studentssubjects form">

        <?php
        echo $this->Form->create('StudentsSubject', array('role' => 'form',
            'inputDefaults' => array(
                'div' => array(
                    'class' => 'form-group'
                ), 'label' => array(
